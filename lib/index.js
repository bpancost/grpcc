...
function createClient(args, options) {
  ...
  let includeDirs = ["/usr/local/include"]
  if (args.directory) {
    includeDirs.push(args.directory)
  }

  let packageDefinition = loader.loadSync(file.file, {includeDirs: includeDirs});
...
function printUsage(pkg, serviceName, address, service, args) {
  ...
  let paths = ["/usr/local/include"]
  if (args.directory) {
    paths.push(args.directory)
  }

  root.resolvePath = function resolvePath(origin, target) {
    let normOrigin = protobuf.util.path.normalize(origin),
    ...
    for (let i = 0; i < paths.length; ++i) {
      let iresolved = protobuf.util.path.resolve(paths[i] + "/", target);
  ...
  return protobuf.load(args.proto, root).then(root => {
